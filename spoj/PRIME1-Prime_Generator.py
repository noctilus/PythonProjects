...
def input_func(input_no_tests):
    test1 = 0
    while test1 < input_no_tests:
        input_test_case.append(input())
        test1 += 1
    return input_test_case


def iteration_func():
    for i in input_test_case:
        range_for_test = i.split()
        for n in range(int(range_for_test[0]), int(range_for_test[1])+1):
            is_prime(n)
        print("\n")


input_func(input_no_tests)
iteration_func()
# 3
